...
export default {
  async createProcedimentType(req, res) {
    ...
    try {
      ...
      const existingProcedimentType =
        await prisma.tipo_de_procedimento.findFirst({
          where: {
            nome,
            clinicaId: clinic.id,
          },
  ...
  async findProcedimentTypesForName(req, res) {
    try {
      ...
      if (procedimentType.length === 0) {
        return res.json({
          error: "Não foram encontrados tipos de procedimento com esse nome.",
        });
      }
  ...
  async updateProcedimentType(req, res) {
    ...
    try {
      ...
      if (!procedimentType) {
        return res.json({
          message: "Não foram encontrados tipos de procedimento com esse ID!",
        });
      }
