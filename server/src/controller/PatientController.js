  ...
  async createPatient(req, res) {
    try {
      ...
      if (!clinic) {
        return res
          .status(400)
          .json({ error: "Não existe clínica com esse id!" });
      }
      ...
      if (existingPatient) {
        return res
          ...
          .json({
            error:
              "Já existe um paciente com esse CPF cadastrado nesta clinica!",
          });
      }
  ...
  async findPatient(req, res) {
    try {
      ...
      if (!patient)
        return res
          .status(400)
          .json({ error: "Não foram encontrados pacientes com esse ID!" });
  ...
  async findPatientForName(req, res) {
    try {
      ...
      if (patient.length === 0) {
        return res
          .status(400)
          .json({ error: "Não foram encontrados pacientes com esse nome." });
      }
    ...
    try {
      ...
      if (patient.length === 0) {
        return res
          ...
          .json({
            error:
              "Não foram encontrados pacientes com essa data de nascimento.",
          });
      }
  ...
  async findPatientForCPF(req, res) {
    try {
      ...
      if (patient.length === 0) {
        return res
          .status(400)
          .json({ error: "Não foram encontrados pacientes com esse CPF." });
      }
