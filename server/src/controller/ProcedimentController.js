...
export default {
  async createProcediment(req, res) {
    ...
    try {
      ...
      if (!clinic) {
        return res.json({
          message: "Não foram encontradas clinincas com esse ID!",
        });
      }

      const procedimentType = await prisma.tipo_de_procedimento.findUnique({
        where: { id: Number(tipo_de_procedimento) },
      });

      if (!procedimentType) {
        return res.json({
          message: "Não foram encontrados tipos de procedimentos com esse ID!",
        });
      }
  ...
  async findProcediment(req, res) {
    try {
      ...
      if (!procediment)
        return res.json({
          error: "Não foram encontrados procedimentos com esse ID!",
        });
  ...
  async findProcedimentsForName(req, res) {
    try {
      ...
      if (procediment.length === 0) {
        return res.json({
          error: "Não foram encontrados procedimentos com esse nome.",
        });
      }
  ...
  async findProcedimentsForType(req, res) {
    try {
      ...
      if (!procedimentType) {
        return res.json({
          error: "Não foram encontrados tipos de procedimentos com esse ID.",
        });
      }
      const procediments = await prisma.procedimento.findMany({
      ...
      if (procediments.length === 0) {
        return res.json({
          error: "Não foram encontrados procedimentos com esse tipo.",
        });
      }
